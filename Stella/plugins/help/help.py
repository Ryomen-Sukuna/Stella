...
async def redirectHelp(client, message):
    if len(message.command) >= 2 and message.command[1].split("_")[0] == "help":
        if len(message.command) >= 2:
...
async def help_button(client, callback_query):
    ...
    if mod_match:
        ...
        await callback_query.message.edit(
            text=html.escape(text), reply_markup=InlineKeyboardMarkup(buttons)
        )
...
async def module_page(module: str, message: str = None):
    ...
    await message.reply(
        text=html.escape(text), reply_markup=InlineKeyboardMarkup(buttons), quote=True
    )
