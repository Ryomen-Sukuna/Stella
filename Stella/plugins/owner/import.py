...
async def StellaImport(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("Reply to exported data!")
        return

    if not (len(COLLECTIONS_DATAS) == 0):
        await message.reply(
            (
    ...
    starting_msg = await message.reply("Started importing job!")
    ...
    for collection in COLLECTIONS_NAMES:
        for exportData in data["exports_data"]:
            ...
            if not get_col == None:
                ...
                import_msg = await starting_msg.edit_text(f"{collection} imported!")
    await import_msg.edit_text("Imported all collectons!")
