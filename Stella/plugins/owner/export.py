...
from Stella import BACKUP_CHAT, OWNER_ID, SUDO_USERS, StellaCli, StellaDB, scheduler
...
async def StellaExport(client, message):
    ...
    if user_id in SUDO_USERS:
        await message.reply("This command is mode for only my kami-samas!")
        return

    collect_collections()
    await StellaCli.send_document(
        chat_id=BACKUP_CHAT, document="Stella_Backup.json", caption="exported!"
    )
...
def exports_in_json(COLLECTIONS_LIST):
    with open("Stella_Backup.json", "w") as backup_file:
        json.dump(
            COLLECTIONS_LIST, backup_file, indent=4, sort_keys=True, cls=DateTimeEncoder
        )


def collect_collections():
    COLLECTIONS_LIST = {"collections": [], "exports_data": []}

    for collection in COLLECTIONS_NAMES:
        COLLECTIONS_LIST["collections"].append(collection)
        all_collec = list(StellaDB[collection].find({}))
        # print(all_collec)
        COLLECTIONS_LIST["exports_data"].append({collection: all_collec})
