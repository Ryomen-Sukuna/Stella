...
async def admin_list(client, message):
    ...
    for user in data_list:
        if user.user.username is not None:
            ADMINS_LIST.append(
                f"- <a href=tg://user?id={user.user.username}>{user.user.first_name}</a> id `{user.user.id}`\n"
            )
        else:
            ADMINS_LIST.append(
                f"- <a href=tg://user?id={user.user.id}>{user.user.first_name}</a> id `{user.user.id}`\n"
            )
    ...
    for admin in ADMINS_LIST:
        admin_header += admin
    await message.reply(
        (f"{admin_header}\n\n" "__These are the updated values.__"), quote=True
    )
