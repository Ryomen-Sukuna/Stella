...
async def fed_ban(client, message):
    ...
    get_user = await StellaCli.get_users(user_ids=userID)

    FED_ADMINS = get_fed_admins(fed_id)

    if userID == BOT_ID:
        await message.reply("Hahahaha no. I am not going to fban myself.")
        return

    if bannedID not in FED_ADMINS:
        await message.reply(f"You aren't a federation admin of {fed_name}.")
        return
    ...
    if is_user_fban(fed_id, userID):
        ...
        for chat_id in connected_chats:
            ...
            if GetData["can_restrict_members"]:
                if await StellaCli.kick_chat_member(chat_id, userID):
