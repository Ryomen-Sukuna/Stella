...
async def FedPromote(client, message):
    ...
    user = await StellaCli.get_users(user_ids=user_id)
        ...
        [
            [
                InlineKeyboardButton(
                    text="Confirm",
                    callback_data=f"fedpromote_promote_{user_id}_{owner_id}",
                ),
                InlineKeyboardButton(
                    text="Cancel",
                    callback_data=f"fedpromote_cancel_{user_id}_{owner_id}",
                ),
            ]
        ]
    )
...
@StellaCli.on_callback_query(
    filters.create(lambda _, __, query: "fedpromote_" in query.data)
)
async def fedpromote_callback(client: StellaCli, callback_query: CallbackQuery):
    ...
    fed_name = get_fed_name(owner_id=owner_id)
    user = await StellaCli.get_users(user_ids=user_id)
    owner = await StellaCli.get_users(user_ids=owner_id)

    if query_data == "promote":
        if user_id == callback_query.from_user.id:
            fed_promote(fed_id, user_id)
            await callback_query.edit_message_text(
                text=(f"User {user.mention} is now admin of {fed_name} ({fed_id})")
            )
        else:
            await callback_query.answer(text=("You aren't the user being promoted."))

    elif query_data == "cancel":
        if user_id == callback_query.from_user.id:
            await callback_query.edit_message_text(
                text=(f"Fedadmin promotion has been refused by {user.mention}.")
            )

        elif owner_id == callback_query.from_user.id:
            await callback_query.edit_message_text(
                text=(f"Fedadmin promotion cancelled by {owner.mention}.")
            )
        else:
            await callback_query.answer(text=("You aren't the user being promoted."))
