...
from Stella.database.federation_mongo import get_fed_name, is_fed_exist, join_fed_db
...
async def JoinFeb(client, message):
    ...
    if not (message.chat.type == "supergroup"):
        await message.reply("Only supergroups can join feds.")
        return
    ...
    if not (await isUserCreator(message)):
        await message.reply("Only Group Creator can join new fed!")
        return

    if not (is_fed_exist(message.command[1])):
        await message.reply("This FedID does not refer to an existing federation.")
        return
