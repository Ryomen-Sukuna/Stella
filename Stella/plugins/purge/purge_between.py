...
from Stella.plugins.purge.purge_helper import PurgeDictData, PurgeDictDataUpdater
...
async def PurgeBetween(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("Reply to a message to show me where to purge from.")
        return
    ...
    if command == "purgefrom":
        ...
        if not message.reply_to_message:
            await message.reply("Reply to a message to let me know what to delete.")
            return
        ...
        if chat_id in PurgeData.keys():
            ...
            ExtraMessageIDs = [message_id, first_messageID, purge_from_messageID]
            ...
            try:
                await StellaCli.delete_messages(
                    chat_id=chat_id, message_ids=MessagesIDs
                )
                PurgeData.clear()
            except:
                await message.reply(
