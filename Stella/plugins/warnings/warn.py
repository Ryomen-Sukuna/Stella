...
async def warn(message, reason, silent=False, warn_user=None):
    ...
    if warn_user is None:
        ...
        if user_id == BOT_ID:
            await message.reply("Bold of you to think I'm gonna warn myself!")
            return
    ...
    if warnchecker == True or warnchecker == None:
        return False
    ...
    if not silent:
        await message.reply(text=warn_text, reply_markup=InlineKeyboardMarkup(button))
        return True
