...
async def generate_api(client, message):
    if not (message.chat.type == "private"):
        await message.reply("You can only get your API key in PM.", quote=True)
        return
    ...
    await message.reply(
        "Tap the button to generate your API key.", reply_markup=button, quote=True
    )


@StellaCli.on_callback_query(
    filters.create(lambda _, __, query: "api_generate" in query.data)
)
async def generate_api_callback(client: StellaCli, callback_query: CallbackQuery):
    ...
    if operation:
        text = "Here is your **StellaGban** API key:\n\n" f"`{api_key}`"
    else:
