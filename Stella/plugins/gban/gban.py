...
async def Gban(client, message):
    ...
    if not (message.from_user.id in SUDO_USERS or message.from_user.id in OWNER_ID):
        return
    ...
    if operation:
        ...
        for chat_id in CHATS_LIST:
            ...
            if GetData["can_restrict_members"]:
                try:
                    if await StellaCli.kick_chat_member(chat_id, GbannedUser):
        ...
        await message.reply(request_message)
