...
async def pin(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("You need to reply to a message to pin it!")
        return
    ...
    if len(message.command) == 1 or (
        len(message.command) >= 2 and message.command[1] in ("silent", "quiet")
    ):
    ...
    elif len(message.command) >= 2 and message.command[1] in (
        "loud",
        "notify",
        "violent",
    ):
