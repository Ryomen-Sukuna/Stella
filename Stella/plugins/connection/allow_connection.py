...
async def allow_connection(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if get_arg in ALLOW_CONNECTION_TRUE:
            ...
            await message.reply("Allow all users in connection.", quote=True)
        elif get_arg in ALLOW_CONNECTION_FALSE:
            allow_collection(chat_id, chat_title, allow_collection=False)
            await message.reply("Disallow all users in connection.", quote=True)
        else:
            await message.reply(f"I got {get_arg} arg!")
    else:
