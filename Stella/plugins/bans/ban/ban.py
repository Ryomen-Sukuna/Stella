...
async def ban(client, message):
    ...
    if user_id == BOT_ID:
        await message.reply("Yeah! I'm not going to ban myself.")
        return

    if not await can_restrict_member(message, user_id):
        await message.reply("I'm not going to ban an admin now. Ffs.")
        return

    await StellaCli.kick_chat_member(chat_id, user_id)
