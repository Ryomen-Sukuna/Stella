...
from Stella.helper.chat_status import can_restrict_member, isBotAdmin, isUserAdmin
...
async def mute(client, message):
    ...
    if not await can_restrict_member(message, user_id):
        await message.reply("Surely I don't plan to ban an admin.")
        return

    await StellaCli.restrict_chat_member(chat_id, user_id, MUTE_PERMISSIONS)
    ...
    if message_id is not None:
        await StellaCli.delete_messages(chat_id=chat_id, message_ids=message_id)
