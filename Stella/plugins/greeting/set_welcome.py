...
async def set_welcome(client, message):
    ...
    if await connection(message) is not None:
        ...
        if message.chat.type == "private":
            await message.reply("This command is only made for grup, not for PM.")
            return
    ...
    if not message.reply_to_message and not len(message.command) > 1:
        await message.reply("You need to give the welcome message some content!")
        return

    CONTENT, TEXT, DATATYPE = GetWelcomeMessage(message)
    print(CONTENT, TEXT, DATATYPE)
    SetWelcome(ChatID, CONTENT, TEXT, DATATYPE)
