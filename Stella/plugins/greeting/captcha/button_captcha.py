...
async def CaptchaButton(chat_id, user_id):
    ...
    if isRuleCaptcha(chat_id) and get_rules(chat_id) is not None:
        captchaButton = [
            [
                InlineKeyboardButton(
                    text=captcha_text,
...
@StellaCli.on_callback_query(
    filters.create(lambda _, __, query: "captcha_" in query.data)
)
    ...
    if message.command[1].split("_")[1] == "button":
        ...
        if new_user_id == user_id:
            ...
            message_id, correct_captcha, chances, captcha_list = (
                GetUserCaptchaMessageIDs(chat_id=new_chat_id, user_id=user_id)
            )
