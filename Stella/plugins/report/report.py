...
async def report_(client, message):
    ...
    if await isUserAdmin(message, silent=True):
        await message.reply("You're an admin here, why'd you need to report someone?")
        return

    if not message.reply_to_message:
        await message.reply("Which user you want to report?")
        return

    if await isUserAdmin(message.reply_to_message, silent=True):
        await message.reply("You can't report admin.")
        return
