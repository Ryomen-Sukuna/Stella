...
def anonadmin_checker(func):
    ...
    async def wrap(client, message: Message):
                ...
                button = [
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data=f"anonAdminConfirm_",
                        )
                    ]
                ]
                await message.reply(
        ...
        @StellaCli.on_callback_query(
            filters.create(lambda _, __, query: "anonAdminConfirm_" in query.data)
        )
        async def anon_callback(client: StellaCli, callback_query: CallbackQuery):
            if await anon_admin_checker(
                callback_query.message.chat.id, callback_query.from_user.id
            ):
                await func(client, message)
                await callback_query.message.delete()
            else:
                await callback_query.answer(text="You are not admin to do that.")
