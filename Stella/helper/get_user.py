    ...
    if message.reply_to_message and not message.forward_from:
        if len(message.command) >= 2:
            ...
            if args.startswith("@") or (
                ...
                user_info = await StellaCli.get_users(user_ids=args)
                return user_info
            else:
        ...
        user_info = await StellaCli.get_users(user_ids=user)
    ...
    if message.reply_to_message:
        if len(message.command) >= 2 and (
            ...
            or (
                ...
                and (len(message.command[1]) >= 5 or len(message.command[1]) <= 15)
            )
        ):
