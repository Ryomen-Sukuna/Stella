...
def lock_db(chat_id, lock_item):
    locks_data = locks.find_one({"chat_id": chat_id})
    ...
    if locks_data is None:
        ...
        locks.update_one({"chat_id": chat_id}, {"$addToSet": {"locked": lock_item}})
...
def unlock_db(chat_id, locked_item):
    locks_data = locks.find_one({"chat_id": chat_id})
    if locks_data is not None:
        locks.update_one({"chat_id": chat_id}, {"$pull": {"locked": locked_item}})
...
def allowlist_db(chat_id, allowlist_arg):
    locks_data = locks.find_one({"chat_id": chat_id})
    if locks_data is not None:
        locks.update({"chat_id": chat_id}, {"$addToSet": {"allow_list": allowlist_arg}})
    else:
