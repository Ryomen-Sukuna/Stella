...
def new_fed_db(new_fed, fed_id, created_time, owner_id):
    GetFed = federation.find_one({"owner_id": owner_id})
...
def is_fed_exist(fed_id=None, owner_id=None) -> bool:
    if fed_id == None:
        ...
        GetFed = federation.find_one({"fed_id": fed_id})
        if GetFed is not None:
            return True
        else:
            return False
...
def join_fed_db(chat_id, chat_title, fed_id):
    federation.update_one(
        ...
        {
            "$addToSet": {
                "chats": {"$each": [{"chat_id": chat_id, "chat_title": chat_title}]}
            }
        },
    )
...
def user_fban(fed_id, user_id, reason):
    federation.update(
        {"fed_id": fed_id},
        {"$set": {"banned_users": [{"user_id": user_id, "reason": reason}]}},
    )
...
def update_reason(fed_id, user_id, new_reason):
    federation.update_one(
        {"fed_id": fed_id, "banned_users.user_id": user_id},
        {"$set": {"banned_users.$.reason": new_reason}},
        False,
        True,
    )
...
def get_fed_from_chat(chat_id):
    for fedData in federation.find({"chats": {"$elemMatch": {"chat_id": chat_id}}}):
...
def get_fed_from_ownerid(owner_id):
    fedData = federation.find_one({"owner_id": owner_id})
    if not fedData == None:
        fed_id = fedData["fed_id"]
        return fed_id
    else:
...
def get_fed_reason(fed_id, user_id):
    fedData = federation.find_one({"fed_id": fed_id})
...
def fed_rename_db(owner_id, fed_name):
    federation.update_one(
        {"owner_id": owner_id}, {"$set": {"fed_name": fed_name}}, False, True
    )
...
def fed_promote(fed_id, user_id):
    federation.update_one(
        {"fed_id": fed_id}, {"$addToSet": {"Admins": {"$each": [user_id]}}}
    )
