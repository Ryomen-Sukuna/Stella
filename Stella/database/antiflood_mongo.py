        ...
        antiflood.insert_one(
            {
                ...
                "flood": flood,
                "flood_mode": {"flood_mode": 1, "until_time": None},
            }
        )
    else:
        if isinstance(flood_arg, int):
            set_key = {"flood_limit": flood_arg, "flood": True}
        elif isinstance(flood_arg, bool):
            set_key = {"flood": flood_arg}
...
def set_antiflood_mode(chat_id, flood_mode, until_time=None):
    ...
    if antiflood_data is not None:
        antiflood.update_one(
            ...
            {
                "$set": {
                    "flood_mode": {"flood_mode": flood_mode, "until_time": until_time}
                }
            },
            upsert=True,
        )
