...
def SetWelcome(chat_id, Content, Text, DataType):
    ...
    WelcomeData = welcome.find_one({"chat_id": chat_id})
    ...
    if WelcomeData == None:
        ...
        welcome.insert_one(WelcomeSetData)
...
def GetWelcome(chat_id):
    ...
    GetWelcomeData = welcome.find_one({"chat_id": chat_id})
...
def SetWelcomeMessageOnOff(chat_id, welcome_message):
    ...
    welcome.update_one({"chat_id": chat_id}, {"$set": {"welcome": welcome_message}})
...
def SetGoodbyeMessageOnOff(chat_id, goodbye_message):
    ...
    welcome.update_one({"chat_id": chat_id}, {"$set": {"goodbye": goodbye_message}})
...
def SetCleanService(chat_id, clean_service):
    ...
    if not (WelcomeData == None):
        ...
        welcome.insert_one(
            {"_id": WelcomeIDs, "chat_id": chat_id, "clean_service": clean_service}
        )
...
def SetCaptcha(chat_id, captcha):
    ...
    GetCaptchaData = welcome.find_one({"chat_id": chat_id})
...
def AppendVerifiedUsers(chat_id, user_id):
    GetVData = welcome.find_one({"chat_id": chat_id})
...
def isUserVerified(chat_id, user_id) -> bool:
    GetData = welcome.find_one({"chat_id": chat_id})
...
def setReCaptcha(chat_id: int, reCaptcha: bool):
    ...
    if getWelcomeData is None:
        ...
        welcome.insert_one({"_id": _id, "chat_id": chat_id, "reCaptcha": reCaptcha})
    else:
        welcome.update(
...
def setRuleCaptcha(chat_id: int, rule_captcha: bool):
    ...
    if getWelcomeData is None:
        _id = welcome.count_documents({}) + 1
        welcome.insert_one(
            {"_id": _id, "chat_id": chat_id, "rule_captcha": rule_captcha}
        )
    else:
        welcome.update(
