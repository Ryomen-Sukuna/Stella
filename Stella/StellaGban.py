...
class StellaClient:
    ...
    def __init__(
        self, api_key: str, host: str = "http://127.0.0.1:5000/api/v1"
    ) -> None:
        self.host = host
    ...
    def gban_protocol(self, admin_id, GbannedUser, reason):
        ...
        GbanData = {"admin": admin_id, "gbanned_user": GbannedUser, "reason": reason}

        ReturnedData = self.session.post(url=URL, json=GbanData)
    ...
    def ungban_protocol(self, user_id):
        ...
        ReturnedData = self.session.delete(url=URL, json=USER_DATA)
        ...
        if ReturnedData.status_code == requests.codes.ok:
            ...
            return ("Server is currently down!", False)

    def banned_list(self) -> list:
        URL = f"{self.host}/admin/users/"

        ReturnedData = self.session.get(url=URL)
    ...
    def generate_api(self, user_id, first_name, username):
        ...
        ReturnedData = self.session.post(url=URL, json=USER_DATA)
